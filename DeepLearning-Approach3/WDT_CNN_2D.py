...
def get_model():
    ...
    model.add(tf.keras.layers.Activation(tf.keras.activations.relu))
    model.add(tf.keras.layers.Conv2D(filters=16, kernel_size=(1, 3)))
    model.add(tf.keras.layers.Activation(tf.keras.activations.relu))
    model.add(tf.keras.layers.Conv2D(filters=32, kernel_size=(1, 3)))
    model.add(tf.keras.layers.Activation(tf.keras.activations.relu))
...
X0_train, X0_test, Y0_train, Y0_test = train_test_split(X, Y0_main, random_state=10, test_size=0.15)

X1_train, X1_test, Y1_train, Y1_test = train_test_split(X, Y1_main, random_state=10, test_size=0.15)

cnn_valence.fit(X0_train, Y0_train, epochs=100, validation_data=(X0_test, Y0_test))
cnn_arousal.fit(X1_train, Y1_train, epochs=150, validation_data=(X1_test, Y1_test))
